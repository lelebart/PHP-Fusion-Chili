...
+--------------------------------------------------------+
| Filename: chili_bbcode_save.php
| Author: slawekneo, lelebart
+--------------------------------------------------------+
...
if ((isset($_GET['thread_id']) && isnum($_GET['thread_id'])) && (isset($_GET['post_id']) && isnum($_GET['post_id'])) && (isset($_GET['code_id']) && isnum($_GET['code_id']))) {
	...
	if (dbrows($result)) {
	    ...
	    preg_match_all("^\[chili=(php|html|css|js|mysql|lotusscript|java|delphi|csharp|cplusplus)([\s][0-9]*|)\](.*?)\[/chili\]^si",$text,$matches,PREG_PATTERN_ORDER);
		$text = unstripinput($matches[3][$_GET['code_id']]);
		$filename = "chili_".$_GET['thread_id']."_".$_GET['post_id']."_".$_GET['code_id'].".txt";
